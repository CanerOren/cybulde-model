...
class ModelStateDictExportingTrainingLightningModule(TrainingLightningModule):
    ...
    def common_export_model_state_dict(self, checkpoint_path: str) -> str:
        # Checkpoint'i yükle (PyTorch 2.6 için weights_only=False önemli)
        ...
        # sadece model ağırlıklarını ayıkla (loss.* olanları düş)
        model_state_dict: dict[str, torch.Tensor] = {}
        for key, value in state_dict.items():
            if not key.startswith("loss."):
                # "model." prefix'ini bir kez çıkar
                new_key = key.replace("model.", "", 1)
                model_state_dict[new_key] = value
        
        # kaydedilecek path
        model_state_dict_save_path = os.path.join(
